...
# packages
packs <- c("dplyr", "nimble", "htmltools", "ggplot2", "sf", "Rcpp", "RcppArmadillo", "inline", "mvtnorm", "parallel")
...
This process was first discussed by @blackwell1973, who describe the relationship between a Pólya sequence and the DP. A sequence $\{\theta_i\}_{i=1}^n$ (for $n \geq 1$) of random values in $\mathcal{X}$ is a Pólya sequence with parameters $\alpha$ and $G_0$ if 
\[
\theta_1 \sim G_0
\]
and 
...
@blackwell1973 showed that when the discrete color palette described in this metaphor is a extended to a continuum of colors and $n \rightarrow \infty$, the Pólya urn scheme converges to a DP. More formally, they showed that if the rules of the Pólya urn scheme described above hold, then 
\begin{itemize}
...
We first simulate data from a univariate normal mixture. 

\[
\begin{split}
y_i &\sim \text{normal}(\mu_{c_i}, \sigma_{c_i}) \\
c_i &\sim \text{multinomial}(\pi)
\end{split}
\]

In this case, we assume $\boldsymbol{\mu} = \{-5,0, 5\}$, $\boldsymbol{\sigma} = \{1.5, 1, 1.5\}$ and $\boldsymbol{\pi} = \{1/3, 1/3, 1/3\}$
...
Next we fit the infinite mixture model and visualize the raw MCMC results, placing a $\text{gamma}(1,1)$ prior on $\alpha$. Formally, the model is
...
\end{split}
\]
where $\text{logit}(\psi_i) = x_i' \beta + \eta_i$, $\boldsymbol{\eta} \sim \mathcal{N}(0, \Sigma)$, and $\Sigma_{ij} = \sigma^2 \exp\left(-\frac{1}{2\rho^2} ||\boldsymbol{s}_i - \boldsymbol{s}_j||^2\right)$.

\singlespacing
```{r, results = "hide", cache = F, echo = F}
# generate mvnorm in C++
code <- '
using namespace Rcpp;
int n = as<int>(n_);
...
region_tmp <- st_make_grid(
  x = st_polygon(list(matrix(c(0,0,14,0,14,14,0,14,0,0),ncol = 2, byrow = TRUE))),
  cellsize = 1
) %>%
  st_as_sf() %>%
  as_tibble() %>%
...
We first visualize traceplots for the default sampler. 
...
To improve the mixing on the spatial parameters, we add a block MH sampler on the spatial random effects.

```{r, eval = T, echo = F}
# load samples
fit <- readRDS("rds files/spom/spom_block.rds")
fit <- list(samples = list(chain1 = fit[[1]][[1]], chain2 = fit[[2]][[1]], chain3 = fit[[3]][[1]]))
